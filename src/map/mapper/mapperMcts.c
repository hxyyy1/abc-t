...
static float Map_MctsNodeScore( Map_Node_t * pNode );
...
static void  Map_MctsCollectRegion_rec( Map_Man_t * p, Map_NodeVec_t * vRegion, Map_Node_t * pNode, int nHops, int nLimit );
static int   Map_MctsCompareNodes( const void * p1, const void * p2 );
...
static float Map_MctsNodeScore( Map_Node_t * pNode )    // 根据节点的时序裕量、引用次数、双极性实现情况和面积代价，计算一个综合优先级分数
{
    float Slack, Crit;
    Slack = Map_MctsNodeSlack( pNode );
    Crit = Slack <= 0.0 ? 10.0f + -Slack : 1.0f / (1.0f + Slack);
    return 100.0f * Crit + 2.0f * pNode->nRefs + (pNode->pCutBest[0] && pNode->pCutBest[1] ? 5.0f : 0.0f) + 0.1f * Map_MctsNodeArea( pNode );
}
...
static void Map_MctsPrintDecision( Map_Man_t * p, Map_MctsDecision_t * pDecision, int iDecision )
{
    ...
    Map_MctsTrace( "    decision %d: node=%d level=%d refs=%d slack=%8.2f combos=%d\n",
        iDecision, pDecision->pNode->Num, pDecision->pNode->Level, pDecision->pNode->nRefs,
        Map_MctsNodeSlack(pDecision->pNode), pDecision->nCombos );
...
{
    int i;
    Map_MctsTrace( "MCTS region %d/%d: root=%d level=%d region_nodes=%d decisions=%d base_area=%11.1f delay_bound=%8.2f\n",
        pCtx->iRegion + 1, pCtx->nRegions, pCtx->pRoot->Num, pCtx->pRoot->Level,
        Map_NodeVecReadSize(pCtx->pRegion->vNodes), pCtx->pRegion->nDecisions, pCtx->AreaBase, pCtx->DelayBound );
    for ( i = 0; i < pCtx->pRegion->nDecisions; i++ )
        Map_MctsPrintDecision( pCtx->p, &pCtx->pRegion->Decisions[i], i );
}

static void Map_MctsCollectRegion_rec( Map_Man_t * p, Map_NodeVec_t * vRegion, Map_Node_t * pNode, int nHops, int nLimit )  // 某个起始节点 pNode 出发，沿着当前 best cut 的叶子往前递归扩展，收集一个局部区域 vRegion
{
    Map_Cut_t * pCut;
    int fPhase, i;
    if ( pNode == NULL || !Map_NodeIsAnd(pNode) || pNode->pRepr )
        return;
    if ( Map_NodeVecReadSize(vRegion) >= nLimit )
        return;
    if ( Map_NodeVecPushUnique( vRegion, pNode ) )
        return;
    if ( nHops == 0 )
        return;
    for ( fPhase = 0; fPhase < 2; fPhase++ )
    {
        ...
        if ( pCut == NULL )
            continue;
        for ( i = 0; i < pCut->nLeaves; i++ )   // 遍历当前 cut 的叶子并递归
            Map_MctsCollectRegion_rec( p, vRegion, pCut->ppLeaves[i], nHops - 1, nLimit );
    }
}

static int Map_MctsCompareNodes( const void * p1, const void * p2 )
{
    Map_Node_t * pNode1, * pNode2;
    float Score1, Score2;
    pNode1 = *(Map_Node_t **)p1;
    pNode2 = *(Map_Node_t **)p2;
    Score1 = Map_MctsNodeScore(pNode1);
    Score2 = Map_MctsNodeScore(pNode2);
    if ( Score1 < Score2 )
        return 1;
    if ( Score1 > Score2 )
        return -1;
    return pNode2->Level - pNode1->Level;
}
...
static void Map_MctsBuildRegion( Map_Man_t * p, Map_Node_t * pRoot, Map_MctsRegion_t * pRegion )    // 先收集一个局部区域里的节点，再从这些节点里挑出值得做 MCTS 决策的节点，为每个节点构造 decision
{
    ...
    Map_MctsCollectRegion_rec( p, pRegion->vNodes, pRoot, 2, p->nMctsRegionMax );
    Map_NodeVecSortByLevel( pRegion->vNodes );
    qsort( Map_NodeVecReadArray(pRegion->vNodes), Map_NodeVecReadSize(pRegion->vNodes), sizeof(Map_Node_t *), Map_MctsCompareNodes );
    nDecs = 0;
    for ( i = 0; i < Map_NodeVecReadSize(pRegion->vNodes) && nDecs < MAP_MCTS_MAX_DECISIONS; i++ )
    {
        pNode = Map_NodeVecReadEntry( pRegion->vNodes, i );
        if ( !Map_NodeIsAnd(pNode) || pNode->pRepr )
            continue;
        if ( Map_MctsBuildDecision( p, pNode, &pRegion->Decisions[nDecs] ) )
            nDecs++;
    }
...
static float Map_MctsEvaluateChoices( Map_MctsCtx_t * pCtx, int * pChoices )
{
    ...
    pRegion = pCtx->pRegion;
#ifdef ABC_USE_OPENMP
#pragma omp critical(map_mcts_eval)
#endif
    {
...
int Map_MappingMctsRefine( Map_Man_t * p )     // 枚举所有 hotspot region，并行/串行执行
{
    ...
    ppHot = ABC_ALLOC( Map_Node_t *, p->vMapObjs->nSize );      // 收集 hot nodes
    nHot = 0;
    for ( i = 0; i < p->vMapObjs->nSize; i++ )
    {
        if ( !Map_NodeIsAnd(p->vMapObjs->pArray[i]) || p->vMapObjs->pArray[i]->pRepr )
            continue;
        ppHot[nHot++] = p->vMapObjs->pArray[i];
    }
    qsort( ppHot, nHot, sizeof(Map_Node_t *), Map_MctsCompareNodes );
    nRegions = Abc_MinInt( nHot, p->nMctsRegionNum );
    pRegions = ABC_CALLOC( Map_MctsRegion_t, nRegions );
    ...
    if ( p->fVerbose )
    {
        ...
        Map_MctsTrace( "MCTS start: hot_nodes=%d region_limit=%d sims_per_region=%d max_region_nodes=%d base_area=%11.1f delay_bound=%8.2f log_file=%s\n",
            nHot, p->nMctsRegionNum, p->nMctsSimNum, p->nMctsRegionMax, AreaBase, DelayBase, MAP_MCTS_LOG_FILE );
    ...
    for ( i = 0; i < nRegions; i++ )
        Map_MctsBuildRegion( p, ppHot[i], &pRegions[i] );
    ...
    if ( !p->fMctsParallel )                                    // 对每个 region 跑搜索：串行或并行
    {
        for ( i = 0; i < nRegions; i++ )
        {
            if ( pRegions[i].nDecisions == 0 )
                continue;
            Map_MctsSearchOneRegion( p, ppHot[i], AreaBase, DelayBase, i, nRegions, &pResults[i] );
        }
    }
    else
    {
...
#pragma omp parallel for schedule(dynamic)
        ...
        for ( i = 0; i < nRegions; i++ )
        {
            ...
            Map_MctsSearchOneRegion( pLocal, pLocal->vMapObjs->pArray[ppHot[i]->Num], AreaBase, DelayBase, i, nRegions, &pResults[i] );
